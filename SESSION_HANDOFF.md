...
### js/imageLoader.js
- **Reversed fallback priority**: generated .webp → PHOTO_MAP → node.img → emoji (was: PHOTO_MAP → generated → node.img → emoji)
...
- **Updated error fallback chain**: `onError` now tries PHOTO_MAP after generated fails (was: tried generated after PHOTO_MAP fails)
- Renamed `onGenError` → `onPhotomapError` to match new flow
